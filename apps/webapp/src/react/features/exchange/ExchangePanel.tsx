...
export function ExchangePanel(props: ExchangePanelProps) {
  ...
  const [subView, setSubView] = useState<"play" | "exchange">("play");
      ...
      <div style={{ display: "flex", gap: 4, padding: "8px 12px", background: "rgba(0,0,0,0.25)", borderBottom: "1px solid rgba(255,255,255,0.04)", marginBottom: 8 }}>
        {([
          ...
          <button key={tab.key} onClick={() => setSubView(tab.key)} style={{
            ...
            background: subView === tab.key ? "rgba(255,178,94,0.12)" : "transparent",
            color: subView === tab.key ? "#ffb25e" : "rgba(255,255,255,0.35)",
            fontSize: 11, fontWeight: 600, cursor: "pointer",
            borderBottom: subView === tab.key ? "2px solid rgba(255,178,94,0.5)" : "2px solid transparent",
          }}>
            {tab.icon} {tab.l}
          </button>
        ))}
