...
export function LiveOpsCampaignCard(props: LiveOpsCampaignCardProps) {
  ...
  if (!props.advanced) {
    const approvalState = asText(approvalSummary.current_state || snapshot.approval_state);
    const gateReady =
      schedulerSummary.ready_for_dispatch === true
        ? t(props.lang, "admin_live_ops_gate_ready")
        : t(props.lang, "admin_live_ops_gate_blocked");
    const latestDispatch = asText(dispatchSummary.status || schedulerSummary.latest_dispatch_state);
    const compactCopy =
      props.lang === "tr"
        ? {
            body: "Normal mod sadece onay, gate ve canli gonderim kararini ekranda tutar.",
            approvalCard: "Onay akisi",
            dispatchCard: "Canli gonderim",
            scheduleCard: "Window",
            warningsCard: "Bloklayici nedenler",
            savedLabel: "Son kayit",
            approvedByLabel: "Onaylayan"
          }
        : {
            body: "Normal mode keeps only approval, gate and live dispatch decisions on screen.",
            approvalCard: "Approval flow",
            dispatchCard: "Live dispatch",
            scheduleCard: "Window",
            warningsCard: "Blocking reasons",
            savedLabel: "Last save",
    ...
    return (
      <section className="akrCard akrCardWide" data-akr-panel-key="panel_admin_live_ops" data-akr-focus-key="live_ops_campaign">
        ...
        <p className="akrMuted">{compactCopy.body}</p>
        <div className="akrDecisionDeck">
          <section className="akrDecisionCard">
            <span className="akrKicker">{compactCopy.approvalCard}</span>
          ...
          <section className="akrDecisionCard">
            <span className="akrKicker">{compactCopy.dispatchCard}</span>
            <strong>{gateReady}</strong>
            <p className="akrMutedLine">
          ...
          <section className="akrDecisionCard">
            <span className="akrKicker">{compactCopy.scheduleCard}</span>
            <strong>{asText(snapshot.segment_key)}</strong>
            <p className="akrMutedLine">
            ...
            <p className="akrMutedLine">
              {compactCopy.savedLabel}: {asText(snapshot.updated_at || snapshot.saved_at)}
            </p>
          </section>
        </div>
        {warnings.length ? (
        ...
        {props.liveOpsCampaignError || props.liveOpsCampaignApprovalError || props.liveOpsCampaignDispatchError ? (
          <p className="akrErrorLine">{props.liveOpsCampaignDispatchError || props.liveOpsCampaignApprovalError || props.liveOpsCampaignError}</p>
        ) : null}
      </section>
    );
  }
  ...
  const selectionTrendFamilyRiskFieldBandDaily = asArray(selectionTrendSummary.family_risk_field_family_band_daily_breakdown);
  const selectionTrendFamilyRiskQueryPathBandDaily = asArray(
    selectionTrendSummary.family_risk_query_segment_path_band_daily_breakdown
  );
  const selectionTrendFamilyRiskAdjustmentPathBandDaily = asArray(
    selectionTrendSummary.family_risk_adjustment_segment_path_band_daily_breakdown
  );
  ...
  return (
    <section className="akrCard akrCardWide" data-akr-panel-key="panel_admin_live_ops" data-akr-focus-key="campaign_editor">
      ...
      <section className="akrMiniPanel" data-akr-focus-key="task_summary">
        ...
        />
        <SelectionFamilyDailyTrendList
          title={t(props.lang, "admin_live_ops_selection_trend_family_risk_field_band_daily_title")}
          rows={selectionTrendFamilyRiskFieldBandDaily}
        />
        <SelectionFamilyDailyTrendList
          title={t(props.lang, "admin_live_ops_selection_trend_family_risk_query_path_band_daily_title")}
          rows={selectionTrendFamilyRiskQueryPathBandDaily}
        />
        <SelectionFamilyDailyTrendList
          title={t(props.lang, "admin_live_ops_selection_trend_family_risk_adjustment_path_band_daily_title")}
