...
test("buildPlayerBridgePayloads produces live player bridge payloads from real shell state", async () => {
  ...
  assert.equal(payloads.combatHud.loopDuelRiskCards?.length, 3);
  ...
  assert.equal(payloads.combatHud.loopDuelSubflowCards?.[0]?.title, "ENTRY");
  ...
  assert.equal(payloads.combatHud.loopLadderFlowPanels?.[0]?.title, "COMMAND");
  assert.equal(payloads.combatHud.loopLadderRiskCards?.length, 3);
  ...
  assert.match(payloads.combatHud.loopLadderRiskPanels?.[2]?.lines?.[3] || "", /HEALTH .*ATTN .*TREND /i);
  assert.equal(payloads.combatHud.loopLadderSubflowCards?.length, 3);
  assert.equal(payloads.combatHud.loopLadderSubflowCards?.[0]?.title, "ENTRY");
  ...
  assert.equal(payloads.combatHud.loopTelemetryFlowPanels?.[0]?.title, "COMMAND");
  assert.equal(payloads.combatHud.loopTelemetryRiskCards?.length, 3);
  ...
  assert.match(payloads.combatHud.loopTelemetryRiskPanels?.[2]?.lines?.[3] || "", /HEALTH .*ATTN .*TREND /i);
  assert.equal(payloads.combatHud.loopTelemetrySubflowCards?.length, 3);
  assert.equal(payloads.combatHud.loopTelemetrySubflowCards?.[0]?.title, "ENTRY");
...
test("buildPlayerBridgePayloads surfaces active vault loop micro panels from selected exchange flow", async () => {
  ...
  assert.equal(payloads.tokenOverview.loopPayoutFlowPanels?.[0]?.title, "COMMAND");
  assert.equal(payloads.tokenOverview.loopPayoutRiskCards?.length, 3);
  ...
  assert.match(payloads.tokenOverview.loopPayoutRiskPanels?.[2]?.lines?.[3] || "", /HEALTH .*ATTN .*TREND /i);
  ...
  assert.equal(payloads.tokenOverview.loopPayoutSubflowCards?.[0]?.title, "ENTRY");
  ...
  assert.equal(payloads.tokenOverview.loopRouteFlowPanels?.[0]?.title, "COMMAND");
  assert.equal(payloads.tokenOverview.loopRouteRiskCards?.length, 3);
  ...
  assert.match(payloads.tokenOverview.loopRouteRiskPanels?.[2]?.lines?.[3] || "", /HEALTH .*ATTN .*TREND /i);
  assert.equal(payloads.tokenOverview.loopRouteSubflowCards?.length, 3);
  assert.equal(payloads.tokenOverview.loopRouteSubflowCards?.[0]?.title, "ENTRY");
  ...
  assert.equal(payloads.tokenOverview.loopPremiumFlowPanels?.[0]?.title, "COMMAND");
  assert.equal(payloads.tokenOverview.loopPremiumRiskCards?.length, 3);
  ...
  assert.match(payloads.tokenOverview.loopPremiumRiskPanels?.[2]?.lines?.[3] || "", /HEALTH .*ATTN .*TREND /i);
  assert.equal(payloads.tokenOverview.loopPremiumSubflowCards?.length, 3);
  assert.equal(payloads.tokenOverview.loopPremiumSubflowCards?.[0]?.title, "ENTRY");
  ...
  assert.equal(payloads.tokenOverview.loopWalletRiskCards?.length, 3);
...
test("buildPlayerBridgePayloads surfaces active tasks loop micro panels from selected mission flow", async () => {
  ...
  assert.equal(payloads.operations.loop.offerFlowPanels?.[0]?.title, "COMMAND");
  assert.equal(payloads.operations.loop.offerRiskCards?.length, 3);
  ...
  assert.match(payloads.operations.loop.offerRiskPanels?.[2]?.lines?.[3] || "", /HEALTH .*ATTN .*TREND /i);
  assert.equal(payloads.operations.loop.offerSubflowCards?.length, 3);
  assert.equal(payloads.operations.loop.offerSubflowCards?.[0]?.title, "ENTRY");
  ...
  assert.equal(payloads.operations.loop.claimFlowPanels?.[0]?.title, "COMMAND");
  assert.equal(payloads.operations.loop.claimRiskCards?.length, 3);
  ...
  assert.match(payloads.operations.loop.claimRiskPanels?.[2]?.lines?.[3] || "", /HEALTH .*ATTN .*TREND /i);
  assert.equal(payloads.operations.loop.claimSubflowCards?.length, 3);
  assert.equal(payloads.operations.loop.claimSubflowCards?.[0]?.title, "ENTRY");
  ...
  assert.equal(payloads.operations.loop.streakFlowPanels?.[0]?.title, "COMMAND");
  assert.equal(payloads.operations.loop.streakRiskCards?.length, 3);
  ...
  assert.match(payloads.operations.loop.streakRiskPanels?.[2]?.lines?.[3] || "", /HEALTH .*ATTN .*TREND /i);
  assert.equal(payloads.operations.loop.streakSubflowCards?.length, 3);
  assert.equal(payloads.operations.loop.streakSubflowCards?.[0]?.title, "ENTRY");
  ...
  assert.equal(payloads.operations.loop.lootRiskCards?.length, 3);
  ...
  assert.equal(payloads.operations.loop.lootSubflowCards?.[0]?.title, "ENTRY");
...
test("buildAdminBridgePayloads produces runtime, asset and audit cards from admin state", async () => {
  ...
  assert.equal(payloads.runtime.loopQueueFlowPanels?.[0]?.title, "COMMAND");
  assert.equal(payloads.runtime.loopQueueRiskCards?.length, 3);
  ...
  assert.match(payloads.runtime.loopQueueRiskPanels?.[2]?.lines?.[3] || "", /HEALTH .*ATTN .*TREND /i);
  assert.equal(payloads.runtime.loopQueueSubflowCards?.length, 3);
  assert.equal(payloads.runtime.loopQueueSubflowCards?.[0]?.title, "ENTRY");
  ...
  assert.equal(payloads.runtime.loopRuntimeFlowPanels?.[0]?.title, "COMMAND");
  assert.equal(payloads.runtime.loopRuntimeRiskCards?.length, 3);
  ...
  assert.match(payloads.runtime.loopRuntimeRiskPanels?.[2]?.lines?.[3] || "", /HEALTH .*ATTN .*TREND /i);
  assert.equal(payloads.runtime.loopRuntimeSubflowCards?.length, 3);
  assert.equal(payloads.runtime.loopRuntimeSubflowCards?.[0]?.title, "ENTRY");
  ...
  assert.equal(payloads.runtime.loopDispatchRiskCards?.length, 3);
  ...
  assert.equal(payloads.runtime.loopDispatchSubflowCards?.[0]?.title, "ENTRY");
